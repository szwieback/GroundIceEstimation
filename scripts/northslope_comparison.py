...
fns_unw_offset = {
    'icecut': {2019: [], 2022: [], 2023: []},
    'happyvalley': {2019: [(7, fngpkg)], 2022: [], 2023: []}}
datesstr = {
    'icecut': {
...
def read_InSAR(site, year):
    ...
    if site == 'happyvalley' and year in (2019, 2022):  # remove first acq because still a lot of snow
        K = K[1:, 1:, ...]
        s_obs = s_obs[1:, ...] - s_obs[0, ...][np.newaxis, ...]
    _geom = geom[site]
    sres = {'doys': doys, 'dates': dates, 'K': K, 's_obs': s_obs, 'geospatial': geospatial, 'geom': _geom}
    return sres
...
def InSAR_results(site, year, rmethod='hadamard', overwrite=False):
    ...
    if not os.path.exists(fnsite) or overwrite:
        ...
        _rc_site = sres['geospatial'].rowcol(xy_site[site])[:, 0]
        C_obs = K[..., _rc_site[0], _rc_site[1]]
        s_obs = s_obs[..., _rc_site[0], _rc_site[1]]
        s_obs_v = s_obs / np.cos(sres['geom']['ia'])
        C_obs_v = C_obs / (np.cos(sres['geom']['ia']) ** 2)
        ...
        siteres = {
            ...
            'dates': sres['dates'], 'doys': sres['doys']}
...
def plot_core(ax, site, year, method=None, c='#000000'):
...
def plot_comparison_2023(fnout=None, overwrite=False):
    from scripts.plotting import prepare_figure, colslist
    from string import ascii_lowercase
    ...
    for jsite, site in enumerate(sites):
        ...
        for jyear, year in enumerate(years):
            print(year, site)
            plot_retrieval(axs[jsite, jyear], res[site][year], c=colslist[2], ymax=ymax)
            plot_core(axs[jsite, jyear], site, year, method=methods[year], c=colslist[0])
            plot_subsidence(axs[jsite, 2], res[site][year], c=colslist[jyear])
            axs[jsite, jyear].text(
                -0.29, 0.50, 'depth $y$ [cm]', ha='right', va='center',
                transform=axs[jsite, jyear].transAxes, rotation=90)
        ...
        axs[jsite, 2].text(
            ...
            transform=axs[jsite, 2].transAxes, rotation=90)
    xlab = ('excess ice $e$ [-]', 'excess ice $e$ [-]', 'date')
    coltitles = ('2023', '2022', 'subsidence')
    for jcol, lab in enumerate(xlab):
        axs[-1, jcol].text(
            0.50, -0.39, lab, ha='center', va='baseline', transform=axs[-1, jcol].transAxes)
        axs[0, jcol].text(
    ...
    xyearlab = 1.03
    axs[0, 2].text(
        xyearlab, 0.30, '2023', ha='left', transform=axs[0, 2].transAxes, c=colslist[0])
    axs[0, 2].text(
        xyearlab, 0.55, '2022', ha='left', transform=axs[0, 2].transAxes, c=colslist[1])
    axs[0, 1].text(
        0.35, 0.85, 'InSAR', ha='left', transform=axs[0, 1].transAxes, c=colslist[2])
    axs[0, 1].text(
        0.35, 0.40, 'cores', ha='left', transform=axs[0, 1].transAxes, c=colslist[0])
    axs[0, 0].text(
        0.80, 0.22, '$y_\\mathrm{f}$', ha='left', transform=axs[0, 0].transAxes, c='#cccccc')
    doy_ticks = (152, 182, 213, 244)
    axs[1, 2].set_xticks(doy_ticks)
    axs[1, 2].set_xticklabels(('Jun', 'Jul', 'Aug', 'Sep'))
    if fnout is None:
...
if __name__ == '__main__':
    ...
    # ind = (do - d0).days
    # print(do)
    # print(comparison_thaw_depth(site=site, year=year, ind=-1))
