...
datesstr = {2023:
                 ('20230602', '20230614', '20230708', '20230720', '20230801', 
                  '20230813', '20230825', '20230906', '20230918')}
# xy_ref = np.array([-166.4759, 65.3511])[:, np.newaxis]
xy_ref = np.array([-139.87542, 67.60733])[:, np.newaxis]
...
def process_oldcrow(year=2023, rmethod='hadamard', remove_last=True):
    ...
    path0 = paths['stacks'] / f'OldCrowA/{year}/proc/{rmethod}/geocoded'
    folder_forcing = paths['forcing'] / 'oldcrow'
    pathout = paths['processed'] / f'oldcrowA/{year}/{rmethod}'
    N = 10000
    Nbatch = 1
