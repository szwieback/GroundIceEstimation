...
from analysis import (
    Geospatial, load_object, save_object, InversionResults, read_K, save_geotiff)
...
#     pathres = f'/export/data/Experiments/gie/processed/tiksi/{year}/hadamard'
#     return pathres
def path_results(sensor='s1', year='2023', rmethod='hadamard'):
    pathres = f'/export/data/Experiments/gie/processed/tiksi3/{sensor}/{year}/{rmethod}'
    return pathres
...
def site_analysis():
    ...
    # ax.set_ylim((0.55, 0))
    # plt.show()
def tiksi_map(years, sensor='s1', rmethod='hadamard', wavelength=0.055, fnout=None, overwrite=True):
    thresh = 4.3e-3
    upscale = 32
    ...
    fig = plt.figure()
    fig.set_size_inches((4.50, 2.65), forward=True)
    ...
    labels = [
        f'a) 2020: excess ice {str(int(ys[0][0]*100))}--{str(int(ys[0][1]*100))} cm',
        f'b) 2020: excess ice {str(int(ys[1][0]*100))}--{str(int(ys[1][1]*100))} cm',
        # f'c) 2024: excess ice {str(int(ys[0][0]*100))}--{str(int(ys[0][1]*100))} cm',
        # f'd) 2024: excess ice {str(int(ys[1][0]*100))}--{str(int(ys[1][1]*100))} cm'
    ]

    for jyear, res in enumerate([res0]):
...
def tiksi_map_profiles(fnout=None, overwrite=True):
    import matplotlib.pyplot as plt
    import matplotlib.gridspec as gridspec
    from scripts.plotting import (
        prepare_figure, cmap_e, colslist, _get_index, contrast, initialize_matplotlib,
...
if __name__ == '__main__':
    ...
    fig_dir = f'/home/jchen20/Dropbox/figures_paper/stacks/tiksi/{site_name}_{sensor}_{stack_method}.png'

    # check_results(years[0], sensor='s1', rmethod='hadamard', wavelength=0.055, overwrite=True)
    tiksi_map(years=years, sensor=sensor, rmethod=stack_method, wavelength=0.055, fnout=fig_dir, overwrite=False)
    ...
    # site_name = 'tiksi'
    # site_name = 'tiksi'
    # fnout_dir = '/export/data/Experiments/gie/processed/tiksi/postproc/'
    # fig_dir = f'/home/jchen20/Dropbox/figures_paper/stacks/{site_name}/tiksi.png'
    # tiksi_map_profiles(fnout=fig_dir, overwrite=False)
