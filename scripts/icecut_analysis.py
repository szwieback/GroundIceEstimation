...
def icecut_map_subsidence(fnout=None, overwrite=True):
    import matplotlib.pyplot as plt
    from analysis import read_K
    ...
    fig, axs = prepare_figure(
        ncols=3, nrows=2, figsize=(1.70, 0.52), sharex=True, sharey=True, left=0.04, right=0.98, 
        bottom=0.18, top=0.95, hspace=0.10, wspace=0.10, remove_spines=False)
    inds = ([(5, -1), (0, -1)], [(6, -1), (0, -1)])
    labels = [
    ...
    ax.plot(
        rc_ref[1], rc_ref[0], linestyle='none', ms=2.5, marker='x',
        mec='#333333', zorder=9)
    add_scalebar(ax, geospatial, length=1000, label='1 km', y=-0.16)
    ...
    for jvar, _var in enumerate(['e', 's']):
        _cp = cparms[_var]
        cax = axs[-1, jvar].inset_axes([0.1, -0.35, 0.60, 0.19])
    ...
    for jax, ax in enumerate(axs[:, 0]):
        ax.text(-0.06, 0.50, years[jax], rotation=90, ha='right', va='center', transform=ax.transAxes)
    if fnout is None:
        plt.show()
    else:
        plt.savefig(fnout)
