...
datesstr = {2023:
                 ('20230614', '20230626', '20230708', '20230720', '20230801', '20230813',
                  '20230825', '20230906', '20230918', '20230930')}
# xy_ref = np.array([-166.4759, 65.3511])[:, np.newaxis]
xy_ref = np.array([-166.45649, 65.34377])[:, np.newaxis]

var_atmo = (4e-3) ** 2
fns_unw_offset = {}


def brevig_forcing(folder_forcing, year, remove_last=True):
    df = load_forcing_merra_subset(folder_forcing)
    d0 = {2023: '2023-05-28'}[year]
    d1 = {2023: '2023-09-30'}[year]
...
def process_brevig(year=2023, rmethod='hadamard', remove_last=True):
    from analysis import (InversionProcessorIS, InversionResultsIS)    
    path0 = paths['stacks'] / f'Brevig/gie/{year}/proc/{rmethod}/geocoded'
    folder_forcing = paths['forcing'] / 'brevig'
    pathout = paths['processed'] / f'brevig/{year}/{rmethod}'
    N = 10000
    Nbatch = 1
    ...
    s_obs, K, geospatial = read_InSAR(
        ...
        fill_nan=True)
    dailytemp, ind_scenes = brevig_forcing(folder_forcing, year=year, remove_last=remove_last)
...
if __name__ == '__main__':
    process_brevig(year=2023, remove_last=False)
