...
indranges = [(ind_scenes[-4], ind_scenes[-1]), (ind_scenes[-3], ind_scenes[-1]), (ind_scenes[-2], ind_scenes[-1])]
...
ir = ip.results(
        ind_scenes, ssim, K, pathout=pathout, ec=ec, n_jobs=-1, memory=False, overwrite=False)
ir.save(pathout / 'ir.p')
ir = IR.from_file(pathout / 'ir.p')
ir.blocksize = 1024

ir_mem = ip.results(
        ind_scenes, ssim, K, pathout=pathout, ec=ec, n_jobs=-1, memory=True, overwrite=False)
...
with h5py.File(pathout / f'{expec[0]}_{expec[1]}.h5', 'r') as f:
    print("Datasets:", list(f.keys()))
    print("\nAttributes:", dict(f.attrs))
    for name in f.keys():
        ds = f[name]
        print(f"\n{name}: shape={ds.shape}, dtype={ds.dtype}, chunks={ds.chunks}")
    res_h5 = f[f'{expec[0]}_{expec[1]}'][:]
    # print(f['periods'][:])
    print(f['dates'][:])
print(res.shape, res_h5.shape)
print(np.nanstd(res - res_h5))
print(np.count_nonzero(np.isfinite(res)), np.count_nonzero(np.isfinite(res_h5)))
