...
datesstr = {2019:
                 ('20190527', '20190608', '20190620', '20190702', '20190714', '20190726',
                  '20190807', '20190819', '20190831', '20190912', '20190924')}
xy_ref = np.array([-162.9664, 67.5749])[:, np.newaxis]
var_atmo = (4e-3) ** 2
fns_unw_offset = {}


def noatak_forcing(folder_forcing, year, remove_last=False):
    df = load_forcing_merra_subset(folder_forcing)
    d0 = {2019: '2019-05-18'}[year]
    d1 = {2019: '2019-09-24'}[year]
...
def process_noatak(year=2019, rmethod='hadamard'):
    from analysis import (InversionProcessorIS, InversionResultsIS)    
    path0 = paths['stacks'] / f'Noatak/{year}/proc/{rmethod}/geocoded'
    folder_forcing = paths['forcing'] / 'noatak'
    pathout = paths['processed'] / f'noatak/{year}/{rmethod}'
    N = 10000
    Nbatch = 1
    ...
    s_obs, K, geospatial = read_InSAR(
        ...
        fill_nan=True)
    dailytemp, ind_scenes = noatak_forcing(folder_forcing, year=year)
    indranges = [(ind_scenes[-5] + 5, ind_scenes[-1])] # from Aug 12
