...
xy_ref = np.array([-162.6194, 66.8640])[:, np.newaxis]
var_atmo = (4e-3) ** 2
fns_unw_offset = {}


def kotzebue_forcing(folder_forcing, year, remove_last=False):
    df = load_forcing_merra_subset(folder_forcing)
    d0 = {2019: '2019-05-25'}[year]
    d1 = {2019: '2019-09-28'}[year]
...
def process_kotzebue(year=2019, rmethod='hadamard'):
    from analysis import (InversionProcessorIS, InversionResultsIS)    
    path0 = paths['stacks'] / f'Kotzebue/gie/{year}/proc/{rmethod}/geocoded'
    folder_forcing = paths['forcing'] / 'kotzebue'
    pathout = paths['processed'] / f'kotzebue/{year}/{rmethod}'
    N = 10000
    Nbatch = 1
    ...
    s_obs, K, geospatial = read_InSAR(
        ...
        fill_nan=True)
    dailytemp, ind_scenes = kotzebue_forcing(folder_forcing, year=year)
    indranges = [(ind_scenes[-4], ind_scenes[-1])] # from Aug 05
