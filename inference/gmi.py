...
class GaussianMixtureDistribution():
    ...
    def quantile(self, quantiles, indices=None, steps=8):
        if indices is None:
        ...
        for j_quantile, _quantile in enumerate(quantiles):
            for j_ind, ind in enumerate(indices):
                ...
                for k in range(self.K):
                    _tempquant[k, ...] = norm.ppf(_quantile, loc=mus[k, ..., j_ind], scale=scales[k, ...])
                search_limits[0,:] = np.min(_tempquant, axis=0)
                search_limits[0,:] -= 0.1 * np.abs(search_limits[0,:])
                search_limits[1,:] = np.max(_tempquant, axis=0)
                search_limits[1,:] += 0.2 * np.abs(search_limits[1,:])  # for K=1 case; keep factors
                for step in range(steps):
                    mid_point = 0.5 * np.sum(search_limits, axis=0)
                    cdf_mid = np.sum(
                        norm.cdf(mid_point, loc=mus[..., j_ind], scale=scales) * self.weights, axis=0)
                    ind_update = cdf_mid < _quantile
                    search_limits[0, ind_update] = mid_point[ind_update]
                    ind_update = cdf_mid >= _quantile
...
if __name__ == '__main__':
    ...
    # junk = rs.normal(scale=0.1, size=(N, 3))
