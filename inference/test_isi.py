...
class PSISTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.lw_out2, self.kss2 = psislw2(self.lw)

    def test_logweights(self):
        # by comparison with original python code
        self.assertLess(np.max(np.abs(self.lw_out - self.lw_out2)), 1e-6)
    
    def test_kss(self):
        self.assertLess(np.max(np.abs(self.kss - self.kss2)), 1e-6)
        
    def test_shape(self):
        self.assertEqual(self.sample.shape, self.lw_out.shape)
        self.assertEqual(self.kss.shape[0], self.sample.shape[1])
        self.assertEqual(self.kss.ndim, 1)

    def test_normalized(self):
        self.assertLess(np.max(np.abs(sumlogs(self.lw_out, axis=0))), 1e-6)
